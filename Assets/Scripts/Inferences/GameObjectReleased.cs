...
namespace MATCH
{
    ...
    {
        public class GameObjectReleased : Inference
        {
            GameObject ObjectToGrab;
            bool ObjectGrabbed;

            public GameObjectReleased(string id, EventHandler callback, GameObject objectToRelease) : base(id, callback)
            {
                ObjectToGrab = objectToRelease;
                ObjectGrabbed = false;

                ObjectManipulator objectManipulator = objectToRelease.GetComponent<ObjectManipulator>();
